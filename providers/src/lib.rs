...
) -> Result<Arc<dyn DynAiProvider>, ProviderError> {
    let provider_config = config
        .providers
        .iter()
        .find(|p| p.name == provider_name && p.enabled)
        .ok_or_else(|| {
            ProviderError::Unsupported(format!("no enabled provider named '{}'", provider_name))
        })?;
    build(provider_config)
}

/// 根据单个 provider 配置构建实例
pub fn build(config: &types::config::AiProvider) -> Result<Arc<dyn DynAiProvider>, ProviderError> {
