...
mod tests {
    ...
    fn temp_manager() -> (TempDir, Manager) {
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path().join("margatroid");
        fs::create_dir_all(&root).unwrap();
        let paths = Arc::new(MargatroidPaths::new(root));
        (tmp, Manager::new(paths).init().unwrap())
    }

    fn make_compose(name: &str) -> ComposeFile {
        ComposeFile {
    ...
    #[test]
    fn init_app_creates_default_when_missing() {
        let (_tmp, mgr) = temp_manager();
        assert!(mgr.paths.app_config().exists());
        let _ = mgr.app_config();
    }

    #[test]
    fn init_app_is_idempotent() {
        let (_tmp, mgr) = temp_manager();
        let first = fs::read_to_string(mgr.paths.app_config()).unwrap();
        // 重新构造并 init，验证不会覆盖已有配置
        let mgr = Manager::new(mgr.paths.clone()).init().unwrap();
        let second = fs::read_to_string(mgr.paths.app_config()).unwrap();
        assert_eq!(first, second);
    }

    #[test]
    fn init_app_rejects_corrupted_toml() {
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path().join("margatroid");
        fs::create_dir_all(&root).unwrap();
        let paths = Arc::new(MargatroidPaths::new(root));
        // 写入损坏的 TOML
        fs::create_dir_all(paths.app_config().parent().unwrap()).unwrap();
        fs::write(paths.app_config(), "not valid toml !!!").unwrap();
        let err = Manager::new(paths).init().unwrap_err();
        assert!(format!("{err:?}").to_lowercase().contains("toml"));
    }

    #[test]
    fn save_app_config_roundtrip() {
        let (_tmp, mgr) = temp_manager();
        mgr.save_app_config().unwrap();
        let _ = Manager::new(mgr.paths.clone()).init().unwrap();
    }

    // ── Workspace ──

    #[test]
    fn create_workspace_from_compose() {
        let (_tmp, mut mgr) = temp_manager();

        let compose = make_compose("test-ws");
        mgr.create_workspace(&compose).unwrap();
        ...
        // 缓存
        assert!(mgr.workspace_config("test-ws").is_some());
        assert!(mgr.list_workspaces().contains(&"test-ws"));
    }
    ...
    fn list_workspaces() {
        ...
        mgr.create_workspace(&make_compose("ws-a")).unwrap();
        mgr.create_workspace(&make_compose("ws-b")).unwrap();

        let list = mgr.list_workspaces();
        assert!(list.contains(&"ws-a"));
        assert!(list.contains(&"ws-b"));
    }
    ...
    fn destroy_workspace() {
        ...
        mgr.create_workspace(&make_compose("to-delete")).unwrap();
        assert!(mgr.list_workspaces().contains(&"to-delete"));

        mgr.destroy_workspace("to-delete").unwrap();
        assert!(!mgr.list_workspaces().contains(&"to-delete"));
        assert!(mgr.workspace_config("to-delete").is_none());
    }

    #[test]
    fn restart_picks_up_existing_workspaces() {
        let (_tmp, mut mgr) = temp_manager();
        mgr.create_workspace(&make_compose("alpha")).unwrap();
        mgr.create_workspace(&make_compose("beta")).unwrap();

        // 模拟重启
        let fresh = Manager::new(mgr.paths.clone()).init().unwrap();

        assert!(fresh.workspace_config("alpha").is_some());
        assert!(fresh.workspace_config("beta").is_some());
    }

    #[test]
    fn save_workspace_config_fails_if_not_in_cache() {
        let (_tmp, mgr) = temp_manager();
        let err = mgr.save_workspace_config("ghost").unwrap_err();
