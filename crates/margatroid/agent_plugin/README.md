...
MCL领域事件进入对应的类型化消息数组。待完成的工具调用不再单独存储：Base Lua从`req.ctx`中最近的
Assistant消息和其后连续的Tool消息推导尚未完成的调用，全部完成时MCL才发送下一次推理请求。
ToolPlugin的`Agent.tools.pending`只负责异步工具请求与响应关联。
