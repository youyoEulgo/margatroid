...
擦除事件读取存储：trait
    清空：私有方法
        签名：clear()
    装填擦除事件：私有方法
        签名：push_boxed(事件体：类型擦除<事件类型:事件特征>)
        行为：将事件体恢复为当前存储的事件类型，并调用事件读取存储::push转移事件所有权
    转换为Any只读引用：私有方法
        签名：as_any() -> Any只读引用
    转换为Any可变引用：私有方法
        签名：as_any_mut() -> Any可变引用
...
事件读取存储注册表：结构体--由World持有
    ...
    注册：crate公开方法
        签名：register<事件类型:事件特征>()
        行为：
            如果类型未注册，创建并注册对应类型的事件读取存储
            如果类型已注册，直接返回
    获取事件读取器：crate公开泛型方法
    ...
    清空：crate公开方法
        ...
        行为：
            遍历所有事件读取存储
            调用事件读取存储::clear，保留已注册的存储
```

公开：
```text
事件队列：结构体--由World持有，对外不暴露字段
    待执行事件：循环数组<事件>--私有，因为需要大量首尾插入弹出操作
    构造：crate公开方法
        签名：new() -> Self
        行为：构造待执行事件为空的事件队列
    拉取事件：crate公开方法
        ...
        行为：
            ...
            循环：有界循环-遍历待执行事件，边界：次数-初始待执行事件数量
                调用pop_event()
                如果返回事件，调用事件读取存储注册表::push_event转移事件所有权
    弹出事件：私有方法
        签名：pop_event() -> 可选<事件>
        行为：
            条件：待执行事件非空
            弹出队首事件
            如果事件剩余延迟帧等于0，返回事件
            否则，将事件剩余延迟帧减1，推到队尾并返回空
    添加事件：私有方法
        签名：push_event(事件体：类型擦除<事件类型:事件特征>, 延迟帧：64位无符号整数)
        行为：将事件体和延迟帧封装为事件并推入事件队列
    发送事件：公开泛型方法
        签名：send_event<事件类型:事件特征>(事件体：事件类型)
        行为：下一次tick执行，调用push_event并将延迟帧设为0
    延迟发送事件：公开泛型方法
        签名：send_event_after<事件类型:事件特征>(事件体：事件类型, 额外延迟帧：64位无符号整数)
        行为：下一次tick之后再等待指定次数的tick，调用push_event并将延迟帧设为额外延迟帧
